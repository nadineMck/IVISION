...
;(function (factory) {
    ...
    THREE.VRController = function (gamepad) {
        ...
        if (buttonNamePrimary === undefined)
            buttonNamePrimary =
                gamepad.buttons.length > 1 ? 'button_1' : 'button_0'
        ...
        this.pollForChanges = function () {
            ...
            buttons.forEach(function (button, i) {
                var controllerAndButtonInfo =
                        controllerInfo + button.name + ' ',
                ...
                if (button.value !== gamepad.buttons[i].value) {
                    ...
                    if (verbosity >= 0.6)
                        console.log(
                            controllerAndButtonInfo + 'value changed',
                            button.value
                        )
                    controller.dispatchEvent({
                ...
                if (button.isTouched !== gamepad.buttons[i].touched) {
                    ...
                    if (verbosity >= 0.5)
                        console.log(
                            controllerAndButtonInfo + 'touch ' + eventAction
                        )
                    controller.dispatchEvent({
                ...
                if (button.isPressed !== gamepad.buttons[i].pressed) {
                    ...
                    if (verbosity >= 0.5)
                        console.log(
                            controllerAndButtonInfo + 'press ' + eventAction
                        )
                    controller.dispatchEvent({
    ...
    THREE.VRController.prototype.setVibe = function (name, intensity) {
        ...
        if (typeof name === 'string') {
            ...
            if (typeof intensity === 'number') channel.intensity = intensity
                ...
                if (typeof channel.intensity === 'number')
    ...
    THREE.VRController.onGamepadConnect = function (gamepad) {
        ...
        window.setTimeout(function () {
            window.dispatchEvent(
                new CustomEvent('vr controller connected', {
                    detail: controller,
                })
            )
        }, 500)
    ...
    OrientationArmModel.prototype.setControllerOrientation = function (
        quaternion
    ) {
    ...
    OrientationArmModel.prototype.update = function () {
        ...
        var headYawQ = this.getHeadYawOrientation_(),
            ...
            angleDelta = this.quatAngle_(
                this.lastControllerQ,
                this.controllerQ
            ),
        ...
        var extensionRatio = this.clamp_(
            (controllerXDeg - 11) / (50 - 11),
            0,
            1
        )
        ...
        // the wrist.
        var totalAngle = this.quatAngle_(
            controllerCameraQ,
        ...
        var lerpValue =
            ...
            (elbowRatio +
                wristRatio *
                    extensionRatio *
                    OrientationArmModel.EXTENSION_RATIO_WEIGHT)
