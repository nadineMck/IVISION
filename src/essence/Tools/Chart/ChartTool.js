...
import * as svg2pdf from 'svg2pdf.js'
  ...
  if (window.innerHeight < 1255) {
    ...
    make: function() {
      ...
      .style( 'height', '100%')
      .html(siteInfo);
      this.optionsDiv = tools.append( 'div' )
      .attr('id', 'optionsDiv')
      .style( 'width', '190px' )
      .style( 'position', 'relative' )
      .style( 'float', 'left')
      ...
      .style( 'display', 'none')
      .html(chartOptions);
      this.contentDiv = tools.append( 'div' )
      .attr('id', 'contentDiv')
      ...
      $('#siteSource').click(function() {
        ...
        for (var selectedSite of selectedSites) {
          ...
          features = {properties: {site: selectedSite}};
      ...
      $('#siteSelect').on('change', function() {
        ...
        siteOptions = new SiteOptions(selectedSites, source, fil, type);
        ToolController_.activeTool.loadChart(siteOptions, [north, east, up], coseismics, offset, stackOn);
      ...
      $('#buttonAdd').click(function() {
        ...
        for (var textSite of textSites) {
          ...
          if ($('#siteSource option:contains('+ textSite +')').length == 1) {
            ...
            features = {properties: {site: textSite}};
            ToolController_.activeTool.use(features);
    ...
    use: function( features, noreset ) {
      ...
      up = this.up;
      neu = [north, east, up];
      coseismics = this.coseismics;
      stackOn = this.stackOn;
      offset = this.offset;
        ...
        if (p == 'site') {
          ...
          for (var i in propInfo[p]) {
            ...
            for (var m in propInfo[p][i]) {
              componentId = 'component' + m + modelId;
    ...
    loadChart: function(siteOptions, neu, coseismics, offset, stackOn) { // sites, source, fil, type, neu, coseismics, offset
      ...
      source = siteOptions.source;
      fil = siteOptions.fil;
      type = siteOptions.type;
      ...
      if (neu.includes('x')) { // increase height if not displaying all charts
        chartHeight = ((this.height - 190) / 2);
      } else {
        chartHeight = ((this.height - 190) / 3);
      }
      ...
      var options = {
        ...
        tooltip: {
          ...
          formatter: function () {
            ...
            dateString = d.getFullYear() + '-' + ('0' + (d.getMonth() + 1)).slice(-2) + '-' + ('0' + d.getDate()).slice(-2) + ' (' + d.getFullYear() + '-' + doy(d) + ')';
      ...
      optionsn = $.extend( true, {}, options );
      optionse = $.extend( true, {}, options );
      optionsu = $.extend( true, {}, options );
      var errorsn = 0;
      var errorse = 0;
      var errorsu = 0;
      var resultsn = [];
            ...
            asyncn[i] = $.ajax({
              ...
              success: function(data) {
                ...
                if (data['err']) {
                  ...
                  legendTitle = $('.highcharts-legend-title').children(0).children(0);
                ...
                if (this.idx == 0) {
                  ...
                  offsetTracen = datan['trace'].map(n => [n[0], n[1] == null ? n[1] : (n[1] + offset*(this.idx-errorsn))]);
                  offsetDatan = datan['data'].map(n => [n[0], n[1] + offset*(this.idx-errorsn)]);
            ...
            asynce[i] = $.ajax({
              ...
              success: function(data) {
                ...
                if (this.idx == 0) {
                  ...
                  offsetTracee = datae['trace'].map(n => [n[0], n[1] == null ? n[1] : (n[1] + offset*(this.idx-errorse))]);
                  offsetDatae = datae['data'].map(n => [n[0], n[1] + offset*(this.idx-errorse)]);
            ...
            asyncu[i] = $.ajax({
              ...
              success: function(data) {
                ...
                if (this.idx == 0) {
                  ...
                  offsetTraceu = datau['trace'].map(n => [n[0], n[1] == null ? n[1] : (n[1] + offset*(this.idx-errorsu))]);
                  offsetDatau = datau['data'].map(n => [n[0], n[1] + offset*(this.idx-errorsu)]);
      ...
      loadLegends = function(series) {
        ...
        Chart0 = Highcharts.chart('chart0', options);
      ...
      if (load) {
        ...
        $.when.apply(this, resultsn).done(function() {
          if (optionsn) {
            ...
            Chart1 = Highcharts.chart('chart1', optionsn);
        ...
        $.when.apply(this, resultse).done(function() {
          if (optionse) {
            ...
            Chart2 = Highcharts.chart('chart2', optionse);
        ...
        $.when.apply(this, resultsu).done(function() {
          if (optionsu) {
            ...
            Chart3 = Highcharts.chart('chart3', optionsu);
  ...
  function resizeChartTool(){
    if (window.innerHeight < 1255) {
      ...
      height = 1200; //recommended min height to avoid scrolling
    }
